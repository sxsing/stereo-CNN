...
function DataHandler:__init( data_version, data_root, util_root, num_tr_img, num_val_img, num_val_loc, batch_size, psz, half_range, gpu)
    ...
    self.file_ids = torch.FloatTensor(torch.FloatStorage(paths.concat(util_root, 'myPerm.bin')))--FloatTensor of size (number of images)
print(self.file_ids)
    self.tr_loc = torch.FloatTensor(torch.FloatStorage(('%s/tr_%d_%d_%d.bin'):format(util_root, num_tr_img, self.psz, self.half_range))):view(-1,5)
		--FloatTensor of size 5*many///usage of view(-1,x) is to reshape in x colons
    ...
    for i=1, num_tr_img+num_val_img do
        ...
        local fn = self.file_ids[i]--filename
        local l_img, r_img, gt_img     --left,right and ground truth
        if data_version == 'kitti2015' then
            ...
            gt_img= image.load(string.format('%s/disp_noc_0/%06d_10.png', data_root, fn), 1, 'byte'):float()
            --print(gt_img:size())
        elseif data_version == 'kitti2012' then
    ...
    self.batch_left = torch.Tensor(self.batch_size, self.nChannel, self.pSize, self.pSize)--left batch
    self.batch_right = torch.Tensor(self.batch_size, self.nChannel, self.pSize, self.pSize+self.half_range*2)--right batch, wider than left
    ...
    for i = 1, num_val_loc do  --num_val_loc is usually 1000 or 10000
        ...
        self.val_label[i] = self.gtdata[img_id][{1,center_y,center_x}]
        --print(self.val_label[i])
    end
...
function DataHandler:next_batch()
    ...
    for idx = 1, self.batch_size do
        ...
        -- label is always half_range + 1 (now it's not!)
        self.batch_label[idx] = self.gtdata[img_id][{1,center_y,center_x}]
    end
